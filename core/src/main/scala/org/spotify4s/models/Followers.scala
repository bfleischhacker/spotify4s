...
object Followers {
  implicit val decoder: Decoder[Followers] = (Decoder.instance(_.get[Option[String]]("href")) |@| Decoder
    .instance(_.get[Int]("total"))).map(Followers.apply)

  implicit val encoder: Encoder[Followers] = Encoder
    .instance(followers => Json
      .obj("href" -> Encoder[Option[String]].apply(followers.href), "total" -> Encoder[Int].apply(followers.total)))
